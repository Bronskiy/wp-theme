...
if ( !function_exists( 'booklovers_template_list_theme_setup' ) ) {
	add_action( 'booklovers_action_before_init_theme', 'booklovers_template_list_theme_setup', 1 );
	function booklovers_template_list_theme_setup() {
		booklovers_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'booklovers')
			));
	}
}

// Template output
if ( !function_exists( 'booklovers_template_list_output' ) ) {
	function booklovers_template_list_output($post_options, $post_data) {
