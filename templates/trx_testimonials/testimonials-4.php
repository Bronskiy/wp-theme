...
if ( !function_exists( 'booklovers_template_testimonials_4_theme_setup' ) ) {
	add_action( 'booklovers_action_before_init_theme', 'booklovers_template_testimonials_4_theme_setup', 1 );
	function booklovers_template_testimonials_4_theme_setup() {
		booklovers_add_template(array(
			...
			'title'  => esc_html__('Testimonials /Style 4/', 'booklovers'),
			'thumb_title'  => esc_html__('Avatar (small)', 'booklovers'),
			'w'		 => 75,
			'h'		 => 75
		));
	}
...
// Template output
if ( !function_exists( 'booklovers_template_testimonials_4_output' ) ) {
	function booklovers_template_testimonials_4_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?> class="sc_testimonial_item<?php echo (!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?>"<?php echo !empty($post_options['tag_css']) ? ' style="'.esc_attr($post_options['tag_css']).'"' : '';?>>
				<div class="sc_testimonial_content"><?php booklovers_show_layout($post_data['post_content']); ?></div>
				<?php if ($post_options['photo']) { ?>
				<div class="sc_testimonial_avatar"><?php booklovers_show_layout($post_options['photo']); ?></div>
				<?php } ?>
