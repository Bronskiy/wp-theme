...
if ( !function_exists( 'booklovers_template_news_announce_theme_setup' ) ) {
	add_action( 'booklovers_action_before_init_theme', 'booklovers_template_news_announce_theme_setup', 1 );
	function booklovers_template_news_announce_theme_setup() {
		booklovers_add_template(array(
			'layout' => 'news-announce',
			'template' => 'news-announce',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Announce/', 'booklovers')
		));

		// Add thumb sizes into list
		booklovers_add_thumb_sizes( array( 'thumb_slug' => 'full', 'add_image_size' => false, 'w' => 1170, 'h' => 659 ) );
		booklovers_add_thumb_sizes( array( 'thumb_slug' => 'big', 'add_image_size' => false, 'w' => 770, 'h' => 434 ) );
		booklovers_add_thumb_sizes( array( 'thumb_slug' => 'medium', 'add_image_size' => false, 'w' => 770, 'h' => 217 ) );
		booklovers_add_thumb_sizes( array( 'thumb_slug' => 'small', 'add_image_size' => false, 'w' => 370, 'h' => 209 ) );
	}
}

// Template output
if ( !function_exists( 'booklovers_template_news_announce_output' ) ) {
	function booklovers_template_news_announce_output($post_options, $post_data) {
		...
		$post_data['post_thumb'] = booklovers_get_resized_image_tag($post_data['post_attachment'], $thumb_sizes['w'], $post_data['post_type']=='product' && booklovers_get_theme_option('crop_product_thumb')=='no' ? null :  $thumb_sizes['h']);
		?><article id="post-<?php echo esc_html($post_data['post_id']); ?>" 
			...
			<div class="post_featured">
				...
				<div class="post_info">
					...
					<h5 class="post_title entry-title"><a href="<?php echo esc_url($post_data['post_link']); ?>" rel="bookmark"><?php booklovers_show_layout($post_data['post_title']); ?></a></h5>
					<?php if ( in_array($post_data['post_type'], array('post', 'attachment')) ) { ?>
						<div class="post_meta">
							<span class="post_meta_author"><?php booklovers_show_layout($post_data['post_author_link']); ?></span>
