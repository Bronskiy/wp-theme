...
if ( !function_exists( 'booklovers_template_news_excerpt_theme_setup' ) ) {
	add_action( 'booklovers_action_before_init_theme', 'booklovers_template_news_excerpt_theme_setup', 1 );
	function booklovers_template_news_excerpt_theme_setup() {
		booklovers_add_template(array(
			'layout' => 'news-excerpt',
			'template' => 'news-excerpt',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Excerpt/', 'booklovers'),
...
// Template output
if ( !function_exists( 'booklovers_template_news_excerpt_output' ) ) {
	function booklovers_template_news_excerpt_output($post_options, $post_data) {
				...
				if ( !in_array($post_data['post_format'], array('link', 'aside', 'status', 'quote')) ) {
					...
					if ($show_title) {
						if (!isset($post_options['links']) || $post_options['links']) {
							?>
							<h5 class="post_title entry-title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php booklovers_show_layout($post_data['post_title']); ?></a></h5>
							<?php
						} else {
							?>
							<h5 class="post_title entry-title"><?php booklovers_show_layout($post_data['post_title']); ?></h5>
							<?php
						}
					}
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(booklovers_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
																								: booklovers_get_custom_option('post_excerpt_maxlength')
																)
											);
						}
					}
