...
if ( !function_exists( 'booklovers_template_events_1_theme_setup' ) ) {
	add_action( 'booklovers_action_before_init_theme', 'booklovers_template_events_1_theme_setup', 1 );
	function booklovers_template_events_1_theme_setup() {
		booklovers_add_template(array(
			'layout' => 'events-1',
			'template' => 'events-1',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 1/', 'booklovers'),
...
// Template output
if ( !function_exists( 'booklovers_template_events_1_output' ) ) {
	function booklovers_template_events_1_output($post_options, $post_data) {
		$show_title = true;
		$parts = explode('_', $post_options['layout']);
		$style = $parts[0];
		$columns = max(1, min(12, empty($parts[1]) ? (!empty($post_options['columns_count']) ? $post_options['columns_count'] : 1) : (int) $parts[1]));
		$date = tribe_get_start_date(null, true, 'M j, Y');
		...
		<div class="sc_events_item_content">
			...
			if ($show_title) {
				if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
					?><h5 class="sc_events_item_title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php booklovers_show_layout($post_data['post_title']); ?></a></h5><?php
				} else {
					?><h5 class="sc_events_item_title"><?php booklovers_show_layout($post_data['post_title']); ?></h5><?php
				}
			}
			?>
				...
				if ($post_data['post_protected']) {
					...
					if ($post_data['post_excerpt']) {
						...
						: '<p>'.trim(booklovers_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
							? $post_options['descr'] 
							: booklovers_get_custom_option('post_excerpt_maxlength_masonry'))).'</p>';
					}
					if (!empty($post_data['post_link']) && !booklovers_param_is_off($post_options['readmore'])) {
						?>

						<a href="<?php echo esc_url($post_data['post_link']); ?>" class="sc_events_item_readmore">
							<?php
								booklovers_show_layout($post_options['readmore']); ?><span class="icon-right"></span></a>
							<?php
						}
					}
					?>
