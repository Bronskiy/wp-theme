...
if ( !function_exists( 'booklovers_template_services_4_theme_setup' ) ) {
	add_action( 'booklovers_action_before_init_theme', 'booklovers_template_services_4_theme_setup', 1 );
	function booklovers_template_services_4_theme_setup() {
		booklovers_add_template(array(
			'layout' => 'services-4',
			'template' => 'services-4',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 4/', 'booklovers')
		));
	}
}

// Template output
if ( !function_exists( 'booklovers_template_services_4_output' ) ) {
	function booklovers_template_services_4_output($post_options, $post_data) {
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				?><span class="sc_services_item_title"><?php booklovers_show_layout($post_data['post_title']); ?></span><?php
				if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
					?></a><?php
				}
				?>
