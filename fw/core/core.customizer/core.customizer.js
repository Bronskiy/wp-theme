...
jQuery(document).ready(function(){
	...
	jQuery('.to_demo_wrap').each(function(idx) {
		"use strict";
		var tab = jQuery(this).parents('.booklovers_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		var info = jQuery(this).parents('.booklovers_options_field_info').attr('id', 'to_demo_scheme_'+scheme).attr('data-scheme', scheme).data('scheme', scheme);
		if (idx==0 && jQuery(this).find('.to_demo_pin').length==1 && booklovers_get_cookie('to_demo_fixed')==1) booklovers_options_fix_demo(info);
	});

	// Stick demo panel
	jQuery(document).on('click', '.booklovers_options_field_info .to_demo_pin', function(e) {
		booklovers_options_fix_demo(jQuery(this).parents('.booklovers_options_field_info'));
		e.preventDefault();
		return false;
	});	

	// Update demo on page load
	booklovers_options_decorate_demo();

	// Update demo on change fields
	jQuery('#partition_schemes_content').on('change', '.booklovers_options_input,.booklovers_options_field input[type="hidden"]', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.booklovers_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		booklovers_options_decorate_demo(scheme);
	});

	// Update demo on blur color fields
	jQuery('#partition_schemes_content').on('blur', '.booklovers_options_input_color', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.booklovers_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
	...
	jQuery('#partition_schemes_content .booklovers_options_tab > ul').sortable({
		update: function(event, ui) {
			var tabs = "";
			ui.item.parents('.booklovers_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? BOOKLOVERS_STORAGE['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
			ui.item.parents('.booklovers_options_tab').find('> .booklovers_options_schemes_order').val(tabs);
		}
	}).disableSelection().after('<input type="hidden" name="booklovers_options_schemes_order" class="booklovers_options_schemes_order" value="">');
});


// When switch tabs - update fixed demo block
function booklovers_init_hidden_elements(panel) {
	"use strict";
	var demo = panel.find('.booklovers_options_field_info[data-scheme]');
	if (demo.length == 0) return;
	var info2 = jQuery('.booklovers_options').find('>.booklovers_options_field_info');
...
function booklovers_options_action_scheme_delete(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.booklovers_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	booklovers_message_confirm(BOOKLOVERS_STORAGE['to_strings']['scheme_delete_confirm'], BOOKLOVERS_STORAGE['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'booklovers_options_scheme_delete',
			nonce: BOOKLOVERS_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.booklovers_options_tab').find('> .booklovers_options_schemes_order').val()
		};
		setTimeout(function(){
			booklovers_message_info(BOOKLOVERS_STORAGE['to_strings']['recompile_styles'], BOOKLOVERS_STORAGE['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(BOOKLOVERS_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				booklovers_message_success(BOOKLOVERS_STORAGE['to_strings']['scheme_delete_complete']+'<br>'+BOOKLOVERS_STORAGE['to_strings']['reload_page'], BOOKLOVERS_STORAGE['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				booklovers_message_warning(BOOKLOVERS_STORAGE['to_strings']['scheme_delete_failed']+':<br>'+rez.error, BOOKLOVERS_STORAGE['to_strings']['scheme_delete']);
			}
		});
...
function booklovers_options_action_scheme_copy(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.booklovers_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	booklovers_message_confirm(BOOKLOVERS_STORAGE['to_strings']['scheme_copy_confirm'], BOOKLOVERS_STORAGE['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'booklovers_options_scheme_copy',
			nonce: BOOKLOVERS_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.booklovers_options_tab').find('> .booklovers_options_schemes_order').val()
		};
		...
		jQuery.post(BOOKLOVERS_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				booklovers_message_success(BOOKLOVERS_STORAGE['to_strings']['scheme_copy_complete']+'<br>'+BOOKLOVERS_STORAGE['to_strings']['reload_page'], BOOKLOVERS_STORAGE['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				booklovers_message_warning(BOOKLOVERS_STORAGE['to_strings']['scheme_copy_failed']+':<br>'+rez.error, BOOKLOVERS_STORAGE['to_strings']['scheme_copy']);
			}
		});
...
function booklovers_options_fix_demo(info) {
	var info2 = info.parents('.booklovers_options').toggleClass('to_demo_fixed').find('>.booklovers_options_field_info');
	if (info2.length == 0) {
		var clonedObj = info.clone();
		clonedObj.removeAttr('id');
		info.parents('.booklovers_options').append(clonedObj);
		info2 = info.parents('.booklovers_options').find('>.booklovers_options_field_info');
	}
	booklovers_set_cookie('to_demo_fixed', jQuery('.booklovers_options').hasClass('to_demo_fixed') ? 1 : 0, 365);
}


// Decorate demo block
function booklovers_options_decorate_demo() {
	"use strict";
	...
	jQuery('.to_demo_body').each(function() {
		...
		var scheme = jQuery(this).parents('.booklovers_options_field_info').data('scheme');
